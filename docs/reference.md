...
| `ws.value` | `Float` | エディタ横のUIで設定したWebSocket URL/JSONパスから、サーバがpushするたびに反映される数値(ADR-0033。ADR-0031のHTTPポーリングを置き換え) |
...
- `scatter` の instanced 描画への昇格(sprite/strip、ADR-0014/0016)は
  `point r |> move v |> fill/glow` か `line/bezier |> outline w |> fill c`
  という特定の連鎖しか認識しない。それ以外の合成子を挟むと無警告で
  O(n) の SDF ループに転落していたが、今はこの場合コンパイラが warning
  診断を出す(ADR-0035)ので、見た目は変わらないまま性能が崖から
  落ちたことに気づける
...
- `line`/`bezier` にはSDFが無い(ADR-0037)。`outline`/`fill`/`glow`(3Dのみ)/
  `scatter` 以外の合成子(`move`/`rot`/`scale`/`warp`/`distort`/`cut`/
  `inter`/`<+>`/`if`/`morph`)と組み合わせるとコンパイルエラーになる
